...
The transaction object mirrors Ethereum's transaction structure with necessary simplifications suited for a university project.
...
| **`signature`** | `v`, `r`, `s` | `v`, `r`, `s` (ECDSA) | DepChain successfully uses the standard Ethereum cryptographic curve (`secp256k1` / Web3j). The signature is broken down into its `v`, `r`, and `s` components in the protobuf, exactly matching Ethereum's transaction signing mechanics. |
...
## 4. Client Implementation & Signing Mechanics

To submit valid transactions under this ECDSA architecture, the client application must execute the following flow:
1. **Load Identity**: The client must load its specific ECDSA `ECKeyPair` from the `.key` file generated by the setup tool.
2. **Payload Construction**: The client constructs the transaction payload (Nonce, To, Value, GasPrice, GasLimit, Data).
3. **Hashing & Signing**: The client mathematically hashes the raw payload using Keccak-256 (`CryptoUtils.keccakHash`) and signs it with its `ECKeyPair` (`CryptoUtils.ecSign`).
4. **Protobuf Packaging**: The client extracts the generated `v`, `r`, and `s` components from the signature and encapsulates them into the `Transaction` protobuf message alongside the raw payload data.
5. **Network Submission**: The transaction is sent over the network layer. Note that the underlying `LinkManager` connection will still authenticate the physical UDP network link using the client's RSA certificate `.p12`, completely isolating the network identity from the blockchain (ECDSA) identity.

## 5. Conclusion

By mapping closely to Ethereum while abstracting away unneeded production complexity (256-bit math, EIP-1559, Bloom filters), we have created a robust, easily extensible transaction model. The migration to standard ECDSA (`secp256k1`) fully enables the required frontrunning-resistant ERC-20 token in Stage 2 while remaining perfectly compatible with the EVM standard.
